...
int main()
{
  KVBinaryTree<std::string, KVBinaryTree<std::string, int> > outer;
  KVBinaryTree<std::string, int> lkv;
  ...
  outer.forEach([](const std::string &key, KVBinaryTree<std::string, int> &val){
  ...
  std::shared_ptr<KVBinaryTree<std::string, int> > pLkv = outer.get("outer1");
  pLkv->del("three");
