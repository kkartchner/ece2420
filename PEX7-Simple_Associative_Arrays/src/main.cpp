...
int main()
{
  KVList<std::string, KVList<std::string, int> > outer;
  KVList<std::string, int> lkv;
  ...
  outer.forEach([](const std::string &key, KVList<std::string, int> &val){
  ...
  std::shared_ptr<KVList<std::string, int> > pLkv = outer.get("outer1");
  pLkv->del("three");
